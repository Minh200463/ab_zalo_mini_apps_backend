...
async function solidPng(color: RGB): Promise<Buffer> {
  const [r, g, b] = color;
  return sharp({
    create: { width: 640, height: 640, channels: 3, background: { r, g, b } },
  })
    .png()
    .toBuffer();
}
...
const CATALOG: CategorySeed[] = [
  {
    ...
    products: [
      { name: "Cà chua bi", kw: "tomato", desc: "Cà chua bi tươi, mọng nước." },
      {
        name: "Rau cải xanh",
        kw: "cabbage,greens",
        desc: "Rau cải xanh sạch theo ngày.",
      },
    ...
    products: [
      {
        name: "Nước cam ép",
        kw: "orange-juice",
        desc: "Nước cam ép nguyên chất.",
      },
      {
        name: "Cà phê rang xay",
        kw: "coffee",
        desc: "Cà phê rang xay đậm vị.",
      },
  ...
  },
  {
    ...
    products: [
      ...
      {
        name: "Tiêu đen",
        kw: "pepper,peppercorn",
        desc: "Tiêu đen nguyên hạt.",
      },
      {
        name: "Nước mắm",
        kw: "fish-sauce,sauce",
        desc: "Nước mắm truyền thống.",
      },
    ],
  },
  {
    ...
    products: [
      {
        ...
        kw: "chips,snack",
        desc: "Snack khoai tây giòn.",
      },
      {
        name: "Bánh bông lan",
        kw: "cake,sponge-cake",
        desc: "Bánh bông lan trứng muối.",
      },
    ],
  },
  {
  ...
  handler: async (req: PayloadRequest) => {
    ...
    for (let s = 0; s < STORES.length; s++) {
      ...
      for (let c = 0; c < seed.catalog.length; c++) {
        ...
        count++;
        const catImageId = await createMedia(
          payload,
          `s${s + 1}-cat-${c + 1}`,
          cat.kw,
          lock++,
